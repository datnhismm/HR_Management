...
def main():
    ...
    with db._conn() as conn:
        ...
        dump_query_to_csv(conn, 'SELECT * FROM users WHERE id = ?', (user_id,), BACKUP_DIR / 'users.csv')
        print('Backing up employees...')
        dump_query_to_csv(conn, 'SELECT * FROM employees WHERE user_id = ?', (user_id,), BACKUP_DIR / 'employees.csv')
        c = conn.cursor()
        ...
        for eid in emp_ids:
            ...
            dump_query_to_csv(conn, 'SELECT * FROM attendance WHERE employee_id = ?', (eid,), BACKUP_DIR / f'attendance_emp_{eid}.csv')
            print('Backing up contracts for employee', eid)
            dump_query_to_csv(conn, 'SELECT * FROM contracts WHERE employee_id = ?', (eid,), BACKUP_DIR / f'contracts_emp_{eid}.csv')
        print('Backing up role_audit...')
        dump_query_to_csv(conn, 'SELECT * FROM role_audit WHERE changed_user_id = ? OR actor_user_id = ?', (user_id, user_id), BACKUP_DIR / 'role_audit.csv')
        print('Backing up imputation_audit...')
        dump_query_to_csv(conn, 'SELECT * FROM imputation_audit WHERE actor_user_id = ?', (user_id,), BACKUP_DIR / 'imputation_audit.csv')
        ...
        c.execute('DELETE FROM role_audit WHERE changed_user_id = ? OR actor_user_id = ?', (user_id, user_id))
