...
DB = os.path.join(os.path.dirname(os.path.dirname(db.__file__)), 'database.db') if hasattr(db, '__file__') else 'hr_management_app/src/database/hr_management.db'
# backup
ts = datetime.datetime.utcnow().strftime('%Y%m%d_%H%M%S')
backup_dir = os.path.join('hr_management_app', 'src', 'db_backups', f'pre_fix_pwd_{ts}')
os.makedirs(backup_dir, exist_ok=True)
shutil.copy2(os.path.join(os.path.dirname(db.__file__), 'hr_management.db'), os.path.join(backup_dir, 'hr_management.db'))
...
with db._conn() as conn:
    c = conn.cursor()
    c.execute('UPDATE users SET password_hash = ?, salt = ? WHERE id = ?', (pwd_hash, salt.hex(), user_id))
    conn.commit()
