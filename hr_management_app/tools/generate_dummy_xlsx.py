...
try:
    ...
    raise SystemExit("openpyxl not installed. Run: python -m pip install openpyxl") from exc

wb = Workbook()
ws = wb.active
ws.title = "employees"
headers = ["NAME", "EMAIL", "DOB", "JOB_TITLE", "ROLE", "YEAR_START", "YEAR_END", "CONTRACT_TYPE"]
ws.append(headers)
for r in rows:
    ws.append(r)
