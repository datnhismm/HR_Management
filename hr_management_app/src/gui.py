...
class EmployeeProfileWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        self.role_combo = ttk.Combobox(frm, textvariable=self.role_var, values=ALLOWED_ROLES, state="readonly")
    ...
    def load_employee(self):
        ...
        can_edit_fields = can_edit_info(self.actor_role, target_role) or (self.actor_user_id == user_id)
        ...
        for w in [self.name_entry, self.dob_entry, self.job_entry, self.role_combo,
                  self.year_start_entry, self.year_end_entry, self.contract_entry, self.save_btn]:
    ...
    def save(self):
        kwargs = {
            ...
            "year_start": int(self.year_start_var.get().strip()) if self.year_start_var.get().strip() else None,
            "year_end": int(self.year_end_var.get().strip()) if self.year_end_var.get().strip() else None,
            "contract_type": self.contract_var.get().strip() or None,
        }
        try:
            update_employee(self.emp_id, **{k: v for k, v in kwargs.items() if v is not None or k == "name"})
...
class EmployeeManagementWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        cols = ("id", "empnum", "name", "job", "role", "year_start", "year_end", "contract", "user_id")
        self.tree = ttk.Treeview(frm, columns=cols, show="headings")
        headings = ["DB ID", "Employee#", "Name", "Job Title", "Role", "Year Start", "Year End", "Contract", "User ID"]
        ...
        self.delete_btn = ttk.Button(btns, text="Delete Selected", command=self.delete_selected)
        self.delete_btn.pack(side="left", padx=4)
        ttk.Button(btns, text="Refresh", command=self.load_employees).pack(side="right", padx=4)
    ...
    def load_employees(self):
        ...
        with _conn() as conn:
            c = conn.cursor()
            c.execute("SELECT id, employee_number, name, job_title, role, year_start, year_end, contract_type, user_id FROM employees ORDER BY employee_number")
    ...
    def delete_selected(self):
        ...
        try:
            if role == "admin":
                ...
                if not other:
                    messagebox.showerror("Error", "No other user to transfer admin role to.", parent=self)
                    return
    ...
    def delete_user(self):
        ...
        try:
            if role == "admin":
                ...
                if not other:
                    messagebox.showerror("Error", "No other user to transfer admin to.", parent=self); return
                choices = "\n".join(f"{u[0]}: {u[1]} ({u[2]})" for u in other)
                tid = simpledialog.askinteger("Transfer Admin", f"Target user id to transfer admin to:\n{choices}", parent=self)
                if not tid:
                    return
...
class HRApp(tk.Tk):
    def __init__(self, employee_id: Optional[int] = None, user_role: str = "engineer", user_id: Optional[int] = None):
        super().__init__()
    ...
    def create_widgets(self):
        ...
        self.name_label = ttk.Label(info_frame, text="Name", font=("Segoe UI", 11, "bold"))
        self.name_label.pack(anchor="w")
        self.job_label = ttk.Label(info_frame, text="Job Title", font=("Segoe UI", 10))
        self.job_label.pack(anchor="w")
        ttk.Button(info_frame, text="View / Edit Profile", command=self.open_own_profile).pack(anchor="w", pady=(6,0))

        self.manage_users_btn = ttk.Button(info_frame, text="Manage Users", command=self.open_manage_users)
        ...
        ttk.Label(summary_frame, text="Month (YYYY-MM)").grid(row=0, column=0, sticky="w")
        self.month_var = tk.StringVar(value=datetime.now().strftime("%Y-%m"))
        ttk.Entry(summary_frame, textvariable=self.month_var, width=12).grid(row=0, column=1, sticky="w", padx=6)
        ttk.Label(summary_frame, text="Hourly wage").grid(row=1, column=0, sticky="w")
        self.wage_var = tk.StringVar(value="0.0")
        ttk.Entry(summary_frame, textvariable=self.wage_var, width=12).grid(row=1, column=1, sticky="w", padx=6)
        self.calc_btn = ttk.Button(summary_frame, text="Calc Month", command=self.calc_month)
        ...
        ttk.Button(btn_frame, text="View Details", command=self.view_selected_contract).pack(side="left", padx=5)
        ...
        self.check_btn = ttk.Button(check_frame, text="Check In", command=self.toggle_check)
        ...
        self.entry_cid = tk.StringVar(); self.entry_eid = tk.StringVar()
        self.entry_start = tk.StringVar(); self.entry_end = tk.StringVar()
        self.entry_terms = tk.StringVar()
        ttk.Label(add_frame, text="Contract ID").grid(row=0, column=0, sticky="w")
        ttk.Entry(add_frame, textvariable=self.entry_cid).grid(row=0, column=1, sticky="ew")
        ttk.Label(add_frame, text="Employee ID").grid(row=1, column=0, sticky="w")
        ttk.Entry(add_frame, textvariable=self.entry_eid).grid(row=1, column=1, sticky="ew")
        ttk.Label(add_frame, text="Start (YYYY-MM-DD)").grid(row=2, column=0, sticky="w")
        ttk.Entry(add_frame, textvariable=self.entry_start).grid(row=2, column=1, sticky="ew")
        ttk.Label(add_frame, text="End (YYYY-MM-DD)").grid(row=3, column=0, sticky="w")
        ttk.Entry(add_frame, textvariable=self.entry_end).grid(row=3, column=1, sticky="ew")
        ttk.Label(add_frame, text="Terms").grid(row=4, column=0, sticky="w")
        ttk.Entry(add_frame, textvariable=self.entry_terms).grid(row=4, column=1, sticky="ew")
        add_frame.columnconfigure(1, weight=1)
        self.add_contract_btn = ttk.Button(add_frame, text="Add Contract", command=self.add_contract)
    ...
    def load_contracts(self):
        ...
        try:
            ...
            for row in rows:
                ...
                self.contracts_list.insert(tk.END, f"{cid} | Emp:{eid} | {start} → {end}")
        except Exception as e:
    ...
    def clear_add_fields(self):
        self.entry_cid.set(""); self.entry_eid.set(""); self.entry_start.set("")
        self.entry_end.set(""); self.entry_terms.set("")
    ...
    def calc_month(self):
        if not self.employee_id:
            ...
            return
        if not can_count_salary(self.user_role):
            messagebox.showerror("Permission Denied", "You do not have permission to use salary counting.", parent=self)
            return
