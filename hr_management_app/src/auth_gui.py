...
class SignUpWindow(tk.Toplevel):
    ...
    def create_widgets(self):
        ...
        row = 0
        ttk.Label(frm, text="Full name:").grid(row=row, column=0, sticky="e"); self.name_var = tk.StringVar(); ttk.Entry(frm, textvariable=self.name_var).grid(row=row, column=1, sticky="ew"); row += 1
        ttk.Label(frm, text="Email:").grid(row=row, column=0, sticky="e"); self.email_var = tk.StringVar(); ttk.Entry(frm, textvariable=self.email_var).grid(row=row, column=1, sticky="ew"); row += 1
        ttk.Label(frm, text="Password:").grid(row=row, column=0, sticky="e"); self.pw_var = tk.StringVar(); ttk.Entry(frm, textvariable=self.pw_var, show="*").grid(row=row, column=1, sticky="ew"); row += 1

        ttk.Label(frm, text="DOB (YYYY-MM-DD):").grid(row=row, column=0, sticky="e"); self.dob_var = tk.StringVar(); ttk.Entry(frm, textvariable=self.dob_var).grid(row=row, column=1, sticky="ew"); row += 1
        ttk.Label(frm, text="Job title:").grid(row=row, column=0, sticky="e"); self.job_var = tk.StringVar(); ttk.Entry(frm, textvariable=self.job_var).grid(row=row, column=1, sticky="ew"); row += 1
        ...
        ttk.Label(frm, text="Year start:").grid(row=row, column=0, sticky="e"); self.year_start_var = tk.StringVar(); ttk.Entry(frm, textvariable=self.year_start_var).grid(row=row, column=1, sticky="ew"); row += 1
        ttk.Label(frm, text="Year end:").grid(row=row, column=0, sticky="e"); self.year_end_var = tk.StringVar(); ttk.Entry(frm, textvariable=self.year_end_var).grid(row=row, column=1, sticky="ew"); row += 1

        ttk.Label(frm, text="Contract type:").grid(row=row, column=0, sticky="e"); self.contract_var = tk.StringVar(); ttk.Entry(frm, textvariable=self.contract_var).grid(row=row, column=1, sticky="ew"); row += 1
        ...
        ttk.Button(pic_frame, text="Choose...", command=self.choose_profile_pic).pack(side="right")
        row += 1

        btn_frame = ttk.Frame(frm)
        btn_frame.grid(row=row, column=0, columnspan=2, pady=(12,0))
        ttk.Button(btn_frame, text="Sign Up", command=self.do_sign_up).pack(side="left", padx=6)
        ttk.Button(btn_frame, text="Cancel", command=self.destroy).pack(side="right", padx=6)

        frm.columnconfigure(1, weight=1)

    def choose_profile_pic(self):
        path = filedialog.askopenfilename(title="Select profile picture", filetypes=[("Images", "*.png;*.jpg;*.jpeg;*.gif"), ("All files", "*.*")])
        if path:
    ...
    def do_sign_up(self):
        ...
        if not name or not email or not pw:
            messagebox.showerror("Error", "Name, email and password are required", parent=self)
            return

        if role == "admin" and get_admin_user():
            messagebox.showerror("Error", "Admin account already exists. Choose another role.", parent=self)
            return
...
class AuthWindow(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Entry(frm, textvariable=self.pw_var, show="*").grid(row=1, column=1, sticky="ew")

        btn_frame = ttk.Frame(frm)
        btn_frame.grid(row=2, column=0, columnspan=2, pady=(12,0))
        ttk.Button(btn_frame, text="Sign In", command=self.sign_in).pack(side="left", padx=6)
        ttk.Button(btn_frame, text="Sign Up", command=self.open_sign_up).pack(side="left", padx=6)
        ttk.Button(btn_frame, text="Forgot Password", command=self.forgot_password).pack(side="left", padx=6)
    ...
    def forgot_password(self):
        email = simpledialog.askstring("Forgot password", "Enter your account email:", parent=self)
        if not email:
            return
        ...
        token_in = simpledialog.askstring("Reset", "Enter the reset token:", parent=self)
        if not token_in:
            return
        new_pw = simpledialog.askstring("Reset", "Enter new password:", show="*", parent=self)
        if not new_pw:
            return
