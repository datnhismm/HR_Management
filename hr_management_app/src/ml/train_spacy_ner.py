...
def main():
    parser = argparse.ArgumentParser(description="Train spaCy NER model for employee fields")
    parser.add_argument("--train", required=True, help="Path to training data in spaCy binary format or JSONL")
    ...
    try:
        ...
        logger.error("spaCy is not installed. Install it with: pip install spacy and download a model (python -m spacy download en_core_web_sm)")
        sys.exit(2)
    ...
    for i in range(args.n_iter):
        losses = {}
        batches = minibatch(examples_spacy, size=compounding(4.0, 32.0, 1.001))
