...
try:
    from database.database import send_email
except Exception as imp_ex:
    print("Failed to import application modules. Make sure you run this from the repository root and that src/ is present.")
    raise
...
def main():
    ...
    try:
        ...
        print(f"Sent test email to {to_addr} (or skipped because SMTP not configured). Check the inbox.")
        return 0
    except Exception:
        logger.exception("Test email send failed")
        print("Failed to send email. See log for details. Common causes: wrong SMTP settings, blocked port, auth failure.")
        return 1
